...
- **Static SEO files** in `public/` served at root: `robots.txt`, `site.webmanifest`, `og-image.png`, PWA icons. **HARTE REGEL: kein llms.txt** (nutzer-regel 2026-09-15, global — Agenten-Lesbarkeit lebt on-page, siehe firmenindex agents.html-Muster). **`sitemap-index.xml` is auto-generated by `@astrojs/sitemap`** (includes every page + blog post) — there is no manual sitemap to edit.
